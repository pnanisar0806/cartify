    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-5 pb-16 pt-12 sm:pt-16">
        ...
        <section className="mt-9" aria-labelledby="results-heading" aria-live="polite" aria-busy={loading}>
          ...
          {ingredients === null ? <div className="rounded-xl border border-dashed border-green-900/20 p-8 text-center"><ListChecks className="mx-auto mb-3 h-7 w-7 text-primary/60" aria-hidden="true" /><p className="text-sm text-slate-500">{loading ? "Turning your recipe into a grocery list…" : "Your ingredients and store links will appear here."}</p></div> : ingredients.length === 0 ? <p className="rounded-xl border bg-white p-6 text-sm text-slate-500">No grocery ingredients found. Try pasting a recipe with an ingredient list.</p> :
            <ul className="divide-y divide-green-900/10 overflow-hidden rounded-xl border border-green-900/10 bg-white">{ingredients.map((ingredient, index) => <li key={`${ingredient}-${index}`} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><span className="flex min-w-0 items-center gap-3 font-medium"><Check className="h-4 w-4 shrink-0 text-primary" aria-hidden="true" /><span className="break-words">{ingredient}</span></span><div className="flex shrink-0 flex-wrap gap-2"><Button asChild variant="outline" className="border-orange-200 bg-orange-50 text-orange-900 hover:bg-orange-100"><a href={getSwiggyUrl(ingredient)} target="_blank" rel="noopener noreferrer" aria-label={`Find ${ingredient} on Swiggy Instamart (opens in new tab)`}>Swiggy Instamart<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></a></Button><Button asChild variant="outline" className="border-yellow-200 bg-yellow-50 text-green-900 hover:bg-yellow-100"><a href={getBlinkitUrl(ingredient)} target="_blank" rel="noopener noreferrer" aria-label={`Find ${ingredient} on Blinkit (opens in new tab)`}>Blinkit<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></a></Button><Button asChild variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-900 hover:bg-emerald-100"><a href={getAmazonFreshUrl(ingredient)} target="_blank" rel="noopener noreferrer" aria-label={`Find ${ingredient} on Amazon Fresh (opens in new tab)`}>Amazon Fresh<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></a></Button></div></li>)}</ul>}
        </section>
